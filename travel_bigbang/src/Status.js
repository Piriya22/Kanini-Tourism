...
export default function BookingStatus() {
        ...
        {bookingData && bookingData.length > 0 ? (
          bookingData.map(({ bookingData, associatedPackage }) => (
            <View key={bookingData.booking_id} style={{ ...styles.card, marginBottom: '20px', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', fontSize: '10px' }}>
              <Text style={{ ...styles.packageName, marginBottom: '5px' }}>Package Name: {associatedPackage.package_name}</Text>
  ...
  // const handleDownloadPdfClick = () => {
  //   const blob = new Blob([<PDFDocument bookingData={bookingData} tourPackage={tourPackage} numberOfPeople={numberOfPeople} />], { type: 'application/pdf' });
    
  //   setTimeout(() => {
  //     saveAs(blob, 'booking_summary.pdf');
  //   }, 3000);
  // };
